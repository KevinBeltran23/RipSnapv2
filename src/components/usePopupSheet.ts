...
export function usePopupSheet({ mode, location, onClose, onChange }: UsePopupSheetProps) {
    ...
    // Initialise state when mode/location changes
    useEffect(() => {
        if (mode === 'add') {
    ...
    // Fetch media for selected category in view mode
    useEffect(() => {
        const fetchCategoryMedia = async () => {
            if (mode !== 'view' || !location?.id || selectedCategory === null) return;

            const categoryName =
                CATEGORY_OPTIONS.find(c => c.id === selectedCategory)?.label || 'Unknown';
    ...
    const handleSheetChanges = useCallback(
        (newIndex: number) => onChange(newIndex),
        [onChange],
    );
    ...
    const handleSubmit = useCallback(async () => {
        ...
        if (!firestoreLocationMetadata?.latitude || !firestoreLocationMetadata?.longitude) {
            Alert.alert('Error', 'Please place a pin on the map to set location coordinates');
            return;
        }
        ...
        try {
            ...
            const newCategoryData = { severity: 'unknown_accessibility' as SeverityLevel };

            const uploadedResults = await Promise.all(
                uploadedMedia.map(m => uploadMedia(m.path, locationId, categoryName, m.type)),
            );

            const newMediaEntries: Media[] = uploadedResults.map(r => ({
                url: r.url,
                path: r.path,
                type: r.type,
                name: r.name,
            ...
            await updateLocation(locationId, {
                categories: { ...existingCategories, [categoryName]: newCategoryData },
    ...
    const openGoogleMaps = useCallback(() => {
        if (location?.googleMapsUrl) Linking.openURL(location.googleMapsUrl);
    }, [location?.googleMapsUrl]);

    const handleAddDataPress = useCallback(() => {
        if (location) {
            setSelectedLocation(location as LocationData);
    ...
    return {
        ...
        hasMoreMedia,
        modalMedia,
        // context values needed for render
        contextSelectedLocation,
        firestoreLocationMetadata,
        isPinPlacementMode,
        isAddingLocation,
