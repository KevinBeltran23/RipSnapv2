...
import {
  useMediaByCategoryQuery,
  mediaQueryKey,
} from '../../services/store/mediaQueries';
import {
  uploadMedia,
...
export function usePopupSheet({
  ...
  const { data: locationMedia = [], isLoading: isLoadingMedia } =
    useMediaByCategoryQuery(
  ...
  const savePinMutation = useMutation({
    mutationFn: async () => {
      ...
      const uploadedResults = await Promise.all(
        uploadedMedia.map(m =>
          uploadMedia(m.path, locationId, categoryName, m.type),
        ),
      );
    ...
    onMutate: async () => {
      ...
      const previousLocations =
        queryClient.getQueryData<LocationData[]>(LOCATIONS_QUERY_KEY);
      ...
      if (
        ...
        queryClient.setQueryData<LocationData[]>(LOCATIONS_QUERY_KEY, old =>
          old ? [...old, optimisticPin] : [optimisticPin],
        );
      }
          ...
          onPress: async () => {
            try {
              ...
              const categoryName =
                ...
                'Unknown';
              queryClient.invalidateQueries({
                queryKey: mediaQueryKey(location.id, categoryName),
              });
              reloadAllLocations();
            } catch (error) {
