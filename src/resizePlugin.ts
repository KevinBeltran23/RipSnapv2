...
export function resize(frame: Frame, width: number, height: number): Int8Array {
  ...
  for (let y = 0; y < targetHeight; y++) {
    for (let x = 0; x < targetWidth; x++) {
      ...
      outputFrame[destIndex] = new Uint8Array(arrayData)[srcIndex + 2]; // R
      outputFrame[destIndex + 1] = new Uint8Array(arrayData)[srcIndex + 1]; // G
      outputFrame[destIndex + 2] = new Uint8Array(arrayData)[srcIndex]; // B
    }
  }
