...
export function AuthProvider({ children }: AuthProviderProps) {
                ...
                const firestoreUnsubscribe = onSnapshot(
                    ...
                    async docSnapshot => {
                        if (docSnapshot.exists()) {
                            ...
                            if (prevIsAdminRef.current !== null && prevIsAdminRef.current !== profile.isAdmin && firebaseUser) {
                                firebaseUser.getIdToken(true).catch(e => console.warn('Token refresh after admin change failed:', e));
                            }
    ...
    const signIn = async (email: string, password: string) => {
        ...
        catch (error) { console.error('Sign in error:', error); throw error; }
    };
    ...
    const signUp = async (email: string, password: string, displayName?: string) => {
        ...
        } catch (error) { console.error('Sign up error:', error); throw error; }
    };

    const signInWithGoogle = async () => {
        try { await GoogleAuth.signInWithGoogle(); }
        catch { /* error already logged in google auth service */ }
    };

    const updateUser = async (data: Partial<User>) => {
        if (!authUser || !user) { console.error('No user logged in to update profile.'); return; }
        try {
        ...
        } catch (error) { console.error('Failed to update profile:', error); throw error; }
    };
    ...
    const forgotPassword = async (email: string) => {
        ...
        catch (error) { console.error('Forgot password error:', error); throw error; }
    };

    return (
        <AuthContext.Provider value={{ authUser, user, loading, signIn, signUp, signOut, signInWithGoogle, forgotPassword, updateUser }}>
            {children}
